...
AetherEMS Console 是 AetherEMS 自有的可选能源管理操作界面，基于 Vue 3、TypeScript、
Composition API 和 Element Plus 构建。它通过 AetherIot 的公开 HTTP/Application 契约
管理 EMS，不属于 AetherIot Kernel，也不得直接读写 SHM 或 SQLite。
...
```bash
docker build -t aetherems-console .
```
