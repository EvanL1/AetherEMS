//! Small local-only Aether composition owned by the downstream distribution.
...
impl CommandDispatcher for NoDeviceDispatcher {
    async fn dispatch(&self, _command: ControlCommand) -> PortResult<CommandReceipt> {
        Err(PortError::new(
            ...
            "no commissioned device driver is configured in AetherEMS",
        ))
    }
}

/// Smallest complete downstream composition with no external services.
...
impl MinimalGateway {
    /// Builds the local-only gateway with device control denied by default.
    ...
    /// Publishes a sample as the acquisition owner in conformance tests.
    #[allow(dead_code)]
