...
For a target-specific artifact, provide a verified released Aether CLI and the
matching target runtime manifest, then build the data-only bundle:

```bash
AETHER_CLI=/path/to/verified/aether \
AETHER_RUNTIME_MANIFEST=/path/to/runtime-manifest.json \
...
independent AetherEMS repository must consume an independently released Aether
Kernel artifact and publish its own Pack artifact and downstream CI evidence
according to [ADR-0001](../../docs/adr/0001-independent-aetherems-distribution.md).
